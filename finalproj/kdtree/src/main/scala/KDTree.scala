...
// } ensuring (ks.forall(k => b.keys.contains(k) || List(excKey).contains(k)))
} ensuring (ks.forall(k => xs.contains(k) || ys.contains(k)))

def listExtractForAll[T](ls: List[T], elem: T, cond: T => Boolean): Unit = {
  require(ls.contains(elem))
  require(ls.forall(cond))
  ls match {
    case Nil() => 
    case Cons(h, t) =>
      assert(cond(h) && t.forall(cond)) 
      assert(h == elem || t.contains(elem))
      if h == elem then assert(cond(elem))
      else listExtractForAll(t, elem, cond)
  }
} ensuring (cond(elem))

def listContentConcatCond[T](ls: List[T], xs: List[T], ys: List[T], cond: T => Boolean): Unit = {
  require(ls.forall(elem => xs.contains(elem) || ys.contains(elem)))
  require(xs.forall(cond))
  require(ys.forall(cond))
  ls match {
    case Nil() => 
    case Cons(h, t) => 
      assert(xs.contains(h) || ys.contains(h))
      if xs.contains(h) then 
        listExtractForAll(xs, h, cond)
      else 
        listExtractForAll(ys, h, cond)
      listContentConcatCond(t, xs, ys, cond) // t.forall(cond)
  }
} ensuring (ls.forall(cond))
...
def equalExceptInferFromLeftErased[T](a: Node[T], b: Node[T], rootKey: Key, excKey: Key): Unit = {
  ...
  listContentConcatCond(a.left.keys, b.left.keys, List(rootKey), cond)
